...
public class TerrainFace 
{
    ...
    public void ConstructMesh()
    {
         ...
         for(int y=0; y<resolution;y++){
            for (int x=0; x<resolution;x++){
                ...
                float unscaledElevation = shapeGenerate.CalculateUnScaledElevation(pointOnUnitSphere);
                vertices[i] = pointOnUnitSphere * shapeGenerate.GetScaledElevation(unscaledElevation);
    ...
    public void UpdateUVs(ColourGenerate generate){
        Vector2[] uv = mesh.uv;
         for(int y=0; y<resolution;y++){
            for (int x=0; x<resolution;x++){
                int i = x + y * resolution;
                ...
                uv[i].x = generate.BiomePercentFromPoint(pointOnUnitSphere);
            }
         }
         mesh.uv = uv;
    }
